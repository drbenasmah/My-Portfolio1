      ...
      if (colorblack) {
        if (BOARD_DEF.board[sqr48[indexOfSqr]] != PIECE_TYPE.NO_PIECE) {
          p = new Piece(
            ...
            r * tilesize,
            tilesize,
            BOARD_DEF.board[sqr48[indexOfSqr]]
          );
        }
        tiles.push(
          new Tile(
...
canvas.onmousedown = function (e) {
  ...
  for (let i = 0; i < tiles.length; i++) {
    ...
    if (collide(mp, t) && tiles[i].piece != null && tiles[i].sqr != SQR.NONE) {
      let pTypeSuper =
        tiles[i].piece.pieceType == PIECE_TYPE.SUPER_RED
          ? PLAYER.P1
          : PLAYER.P2;

      if (BOARD_DEF.move != tiles[i].piece.pieceType) {
        if (pTypeSuper != BOARD_DEF.move) break;
      }
...
function loop() {
  ...
  if (counterAlert > 30) {
    let p =
      BOARD_DEF.move == PLAYER.P1
        ? "You won 🎉. Congrats"
        : "AI won. Better Luck Next time";
    stopLoop = true;
    const winnerMessage = document.getElementById("winner-message");
    winnerMessage.textContent = p;
    openModalFunction();
  }
