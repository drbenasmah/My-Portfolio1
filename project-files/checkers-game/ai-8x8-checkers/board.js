...
imgs[1].src = "img/red-plastic.png";
imgs[2].src = "img/blue-plastic.png";
imgs[3].src = "img/red-crown.png";
imgs[4].src = "img/blue-crown.png";
...
const OFF_BOARD = [
  0, 1, 2, 3, 4, 5, 6, 12, 18, 24, 30, 36, 42, 48, 54, 11, 17, 23, 29, 35, 41,
  47, 53, 59, 55, 56, 57, 58,
];
...
function movePiece(src, target, bf) {
  if (
    ...
    bf.board[target] == PIECE_TYPE.NO_PIECE
  ) {
...
function generateMove(player, bf) {
  ...
  for (let i = 0; i < pieceCount; i++) {
    ...
    if (
      bf.board[pieces[i]] == PIECE_TYPE.SUPER_YELLOW ||
      bf.board[pieces[i]] == PIECE_TYPE.SUPER_RED
    ) {
      superCap = superPieceCapture(m, pieces[i], bf) ? true : superCap;
      hasCaptureMove = hasCaptureMove ? true : superCap;
    }

    hasCaptureMove = checkCaptureMoves(m, move1, player, 0, bf)
      ? true
      : hasCaptureMove;
    hasCaptureMove = checkCaptureMoves(m, move2, player, 1, bf)
      ? true
      : hasCaptureMove;
    hasCaptureMove = checkCaptureMoves(m, move3, player, 2, bf)
      ? true
      : hasCaptureMove;
    hasCaptureMove = checkCaptureMoves(m, move4, player, 3, bf)
      ? true
      : hasCaptureMove;

    if (m.captures.length > 0) possibleMoves.push(m);
  }
...
function superPieceCapture(piece, sqr, bf, selectedSquare) {
  ...
  // Check and remember if there is a capture in each direction
  ...
  if (hasCap && selectedSquare) {
    // Handle the piece's move after capture
    let move = piece.move(sqr, selectedSquare);
    if (move == null) return false;
    if (!updatePiecesPos(piece, move, selectedSquare, bf)) return false;
  }
...
function sPieceCap(direct, i, piece, bf) {
  ...
  while (!inOffset(direct)) {
    if (bf.board[direct] != PIECE_TYPE.NO_PIECE) {
      let sPieceType =
        bf.move == PLAYER.P1 ? PIECE_TYPE.SUPER_RED : PIECE_TYPE.SUPER_YELLOW;
...
function sqrPieceEvenOrOdd(sqr) {
  if (EVEN_RANK_SQR.includes(sqr)) return [-6, -7, 5, 6];
...
function checkCaptureMoves(m, move, player, tmove, bf) {
  if (
    bf.board[move] != PIECE_TYPE.NO_PIECE &&
    player != bf.board[move] &&
    !inOffset(move)
  ) {
    let moveTemp = moveCount(player, move);

    let sPieceType =
      player == PLAYER.P1 ? PIECE_TYPE.SUPER_RED : PIECE_TYPE.SUPER_YELLOW;

    if (bf.board[move] == sPieceType) return false;

    if (
      bf.board[move + moveTemp[tmove]] == PIECE_TYPE.NO_PIECE &&
      !inOffset(move + moveTemp[tmove])
    ) {
